...
def operate(c1, op, c2):
    ...
    # binary operations return a dyad ...
        ...
        if usephasors:   # no simple phasor method for addition, so convert to vector
            v1 = PhasorToVector(c1)
            v2 = PhasorToVector(c2)
        if op == '+': 
            c3.a = v1.a + v2.a
            c3.b = v1.b + v2.b
        else:
...
while len(s1) != 0:
    v1 = parseComplex(s1)
    s2 = raw_input('Enter an operation and (optionally) another complex number in the same format: ')
    op = parseOp(s2)
    if op in ops:
        if op in ('|', '@'):
            v2 = Cvector()   # (0,0)
